...
- `--parallel` — Enable parallel execution via subagents
- `--slug <name>` — Ideation project slug (e.g., `doctor-evolution`). Resolves to spec file via ideation-output directory.
- `--phase <n>` — Phase number (e.g., `1`, `2`). Requires `--slug`. Selects `{slug}-spec-phase-{n}.md`.
- Positional argument — Full file path to a spec (legacy, still supported)

Examples:
- `/ideation:execute-spec --slug doctor-evolution --phase 2`
- `/ideation:execute-spec --slug doctor-evolution` (lists phases to pick)
- `/ideation:execute-spec --parallel --slug doctor-evolution --phase 1`
- `/ideation:execute-spec /full/path/to/spec.md`
...
**If positional file path provided** (contains `/` or `.md`): Read the spec file directly.

**If `--slug` provided:**

1. Resolve the ideation output directory: check CLAUDE.md files for `ideation-output` directive. If not set, use `./docs/ideation` (relative to cwd).
2. If `ideation-output` is set (remote vault), derive `{repo-name}` from cwd:
   - Run `git rev-parse --git-common-dir`. If absolute path (bare repo), take parent directory's basename.
   - Otherwise, `basename(git rev-parse --show-toplevel)`.
   - Fallback: `basename(cwd)`.
   - Search `{ideation-output}/{repo-name}/{slug}/`
3. If using local default (`./docs/ideation`), search `./docs/ideation/{slug}/`
4. In either case, look for files matching `*-spec-phase-*.md`, `*-spec.md`, or legacy `spec-phase-*.md`
5. If `--phase` provided, select the file matching phase `{n}`
6. If no `--phase` and multiple specs found, use `AskUserQuestion` to select one
7. If no specs found under `{repo-name}`, broaden search to `{ideation-output}/*/{slug}/` (slug may belong to a different repo)

**If no arguments:** Auto-detect from task list:
...
1. First check `./docs/ideation/*-spec-phase-*.md` and `./docs/ideation/*/spec-phase-*.md` (local project, new and legacy naming)
2. If none found, resolve ideation-output and repo-name as above, then search `{ideation-output}/{repo-name}/*-spec-phase-*.md`
