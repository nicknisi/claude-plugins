...
All artifacts are written to a configurable output directory. Set `ideation-output: /absolute/path` in any CLAUDE.md to route artifacts to a central location (e.g., an Obsidian vault). When set, artifacts are organized as `{ideation-output}/{repo-name}/{slug}/`. When not set, defaults to `./docs/ideation/{slug}/`.

All filenames are slug-prefixed to prevent wikilink ambiguity:

```
{slug}-contract.md                    # Problem, goals, success criteria, scope, execution plan
{slug}-prd-phase-1.md                 # Phase 1 requirements (only if PRDs chosen)
{slug}-spec-phase-1.md                # Phase 1 implementation spec (with failure modes)
{slug}-spec-template-{pattern}.md     # Shared template for repeatable phases (if applicable)
{slug}-spec-phase-N.md                # Per-phase delta or full spec
```

Artifacts include YAML frontmatter with tags (`ideation`, `ideation/contract`, `ideation/spec`, `ideation/prd`, `ideation/context-map`) and wikilinks between related artifacts (specs link to contracts, etc.).
...
Contracts track revision history via a `Supersedes` field. When re-running ideation on the same project, the prior contract is renamed to `{slug}-contract-{date}.md` and the new contract references it via wikilink, creating a traceable revision chain.
...
flowchart TD
    subgraph IDEATION["<b>Ideation Skill</b> — Planning"]
        ...
        K --> L["📄 Artifacts in<br/>configured output dir"]
    end
...
```bash
# By slug and phase (resolves via ideation-output or ./docs/ideation/)
/ideation:execute-spec --slug doctor-evolution --phase 2

# By slug (lists available phases to pick from)
/ideation:execute-spec --slug doctor-evolution

# Auto-detect next unblocked task from TaskList
/ideation:execute-spec

# Full file path (legacy)
/ideation:execute-spec /path/to/spec.md

# Parallel: spawn subagents for independent tasks
/ideation:execute-spec --parallel --slug doctor-evolution --phase 1
```
